...
sip_do_generate() {
	...
	do
		cd ${S}
		install -d ${module}/
		echo "calling 'sip4 -I sip -I ${STAGING_SIPDIR} ${SIPTAGS} ${FEATURES} -c ${module} -b ${module}/${module}.sbf sip/${module}/${module}mod.sip'"
		...
		sed -e 's,target,TARGET,' -e 's,sources,SOURCES,' -e 's,headers,HEADERS,' \
			${S}/${module}/${module}.sbf | sed s,"moc_HEADERS =","HEADERS +=", \
			> ${S}/${module}/${module}.pro
